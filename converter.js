/**
 * Bangala Unicode ⇄ Bijoy (ANSI) Conversion Engine
 * Contains all conversion maps, helpers, and the core conversion functions.
 */

// ===============================================
// CONVERSION MAPS (Unicode → Bijoy / ANSI)
// ===============================================
...
// Reverse maps (Bijoy → Unicode) – kept for completeness
...
function applyConversionMap(input, conversion_map) {
  ...
  for (var i = 0; i < keys.length; i++) {
    var key = keys[i];
    if (key.length === 1 && /\s/.test(key)) continue;
...
function MapKarToSorborno(CUni) {
  var map = {
    "া":"আ","ি":"ই","ী":"ঈ","ু":"উ","ূ":"ঊ","ৃ":"ঋ","ে":"এ","ৈ":"ঐ","ো":"ও","ো":"ও","ৌ":"ঔ","ৌ":"ঔ"
  };
  return map[CUni] || CUni;
}
...
// Rearrangement for Bijoy → Unicode
...
// Rearrangement for Unicode → Bijoy
...
function ConvertToASCII(ConvertTo, line) {
  ...
  if (ConvertTo == "boisakhi") {
    // re-apply for Boisakhi specific adjustments
    line = line.replace(/ো/g, "ো");
    line = line.replace(/ৌ/g, "ৌ");
    line = ReArrangeUnicodeText(line);
    line = applyConversionMap(line, uni2boisakhi_string_conversion_map);
  }
  return line;
}